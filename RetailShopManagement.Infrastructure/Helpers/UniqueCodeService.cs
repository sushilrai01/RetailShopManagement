...
namespace RetailShopManagement.Infrastructure.Helpers
{
    public class UniqueCodeService(IDbContextFactory<ApplicationDbContext> contextFactory) : IUniqueCodeService
    {
        ...
        public async Task<string> GetUniquePurchaseOrderInvoiceNumberAsync(CancellationToken cancellationToken = default)
        {
            await using var context = await contextFactory.CreateDbContextAsync(cancellationToken);
            ...
            {
                string yearMonth = DateTime.Now.ToString("yyyyMM");
                string prefix = $"RP-{yearMonth}-";

                // Get the last invoice number for this month
                var lastInvoice = await context.Invoices
                    .Where(i => i.IsPurchaseInvoice && i.InvoiceNumber.StartsWith(prefix))
                    .OrderByDescending(i => i.InvoiceNumber)
                    .Select(i => i.InvoiceNumber)
                    .FirstOrDefaultAsync(cancellationToken);
